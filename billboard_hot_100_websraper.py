...
for container in containers:
    ...
    last_week_container = container.find(
        "div", {"class": "chart-row__last-week"})
    last_week = last_week_container.find(
        "span", {"class": "chart-row__value"}).text

    # Grabs the song's peak position
    peak_position_container = container.find(
        "div", {"class": "chart-row__top-spot"})
    peak_position = peak_position_container.find(
        "span", {"class": "chart-row__value"}).text

    # Grabs the song's duration in the hot 100 (in weeks)
    weeks_on_chart_container = container.find(
        "div", {"class": "chart-row__weeks-on-chart"})
    weeks_on_chart = weeks_on_chart_container.find(
        "span", {"class": "chart-row__value"}).text
